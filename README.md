...
3:00 / 2      = 1:30
1:00 / 0:15   = 4
40:00 / 7 * 31 = 177:08 （分未満の端数あり）
```

## 主な機能

- 電卓風のボタンUIで数式を入力（数字・`:`・`+ − × ÷`・`( )`・`=`・`AC`・`⌫`）
...
- 表示は `H:MM` 形式（時は24で折り返さない、経過時間としての表示）
- 内部計算は秒単位で保持しており、結果に分未満の端数が残る場合は分の部分に下線を表示（将来的な秒の入力・表示への対応を見据えた実装）
...
- 秒の入力・表示UI（内部エンジンは対応済みで、UIの拡張のみで対応可能）
- キーボードからの数式入力
...
- **`calculatorInput.ts`**: ボタン入力（数字・`:`・演算子・括弧・`=`・`AC`・`⌫`）を受け取り、数式文字列を組み立てる純粋な状態機械。括弧の自動補完、コロン以降の分の桁数チェック、`)` の直後の暗黙の乗算禁止、単項マイナス、`=` 後に演算子を押した場合の続け計算などを担当し、`timeCalc.ts` の計算ロジックとは独立してテストできるようにしている。
- **`Calculator.tsx`**: 上記2つのモジュールを使い、ボタングリッドと数式・結果の表示を行う。結果が時間で分未満の端数を持つ場合、分の部分を下線付きで表示する。また、数式表示エリアのタップ位置を文字インデックスに変換し、カーソル移動として扱う。
